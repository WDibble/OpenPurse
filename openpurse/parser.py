...
class OpenPurseParser:
        ...
        for pm_el in self._get_nodes('//ns:PmtInf'):
            ...
            pm = {
                ...
                "debtor_name": (
                    ...
                    if "pain.001" in ns_str else 
                    self._get_text_from(pm_el, './ns:Cdtr/ns:Nm/text()')
                ),
                "debtor_account": (
                    self._get_text_from(
                        pm_el, 
    ...
    def _parse_pain002_detailed(self, base_msg: PaymentMessage) -> Pain002Message:
        ...
        for tx_el in self._get_nodes('//ns:TxInfAndSts'):
            tx = {
                "original_instruction_id": self._get_text_from(
                    tx_el, './ns:OrgnlInstrId/text()'
                ),
                "original_end_to_end_id": self._get_text_from(
                    tx_el, './ns:OrgnlEndToEndId/text()'
                ),
                "transaction_status": self._get_text_from(
                    tx_el, './ns:TxSts/text()'
                ),
                ...
                "original_amount": self._get_text_from(
                    tx_el, './/ns:OrgnlTxRef/ns:Amt//text()'
                )
            }
            statuses.append(tx)
        ...
        return Pain002Message(
            **base_msg.to_dict(),
            ...
            original_message_id=self._get_text(
                '//ns:OrgnlGrpInfAndSts/ns:OrgnlMsgId/text()'
            ),
            original_message_name_id=self._get_text(
                '//ns:OrgnlGrpInfAndSts/ns:OrgnlMsgNmId/text()'
            ),
            group_status=self._get_text(
                '//ns:OrgnlGrpInfAndSts/ns:GrpSts/text()'
            ),
